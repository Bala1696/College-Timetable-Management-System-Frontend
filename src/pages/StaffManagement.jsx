                                ...
                                filteredStaff.map((staff, index) => (
                                    <tr key={staff.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 font-medium flex items-center gap-3">
                                            {staff.profilePhoto ? (
                                                <img
                                                    src={staff.profilePhoto.startsWith('http') ? staff.profilePhoto : `http://localhost:5000/uploads/${staff.profilePhoto}`}
                                                    alt={staff.name}
                                                    className="w-10 h-10 rounded-full object-cover cursor-pointer hover:ring-2 hover:ring-purple-400 transition-all"
                                                    onClick={(e) => { e.stopPropagation(); setPreviewImage(staff.profilePhoto.startsWith('http') ? staff.profilePhoto : `http://localhost:5000/uploads/${staff.profilePhoto}`); }}
                                                />
                                            ) : (
