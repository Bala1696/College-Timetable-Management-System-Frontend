...
const FacultyManagement = () => {
    ...
    const canEdit = ['admin', 'faculty'].includes(userRole);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing) {
                await api.put(`/admin/faculty/${formData.id}`, data, {
        ...
        <div className="p-6 space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500">Manage faculty members, their details and access.</p>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <Button onClick={() => openModal()}><Plus className="w-4 h-4 mr-2" /> Add Faculty</Button>
                </div>
            </div>
                    ...
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr><td colSpan="7" className="text-center p-4">No faculty found</td></tr>
                            ) : (
                                filteredFaculty.map((faculty, index) => (
                                    <tr key={faculty.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4"><span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-semibold">{faculty.designation}</span></td>
                                        <td className="px-6 py-4">{faculty.qualification}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span>{faculty.email}</span>
                                                <span className="text-xs text-gray-500">{faculty.mobileNo}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">{faculty.teachingExp || '-'}</td>
                                        <td className="px-6 py-4 flex gap-2">
                                            {canEdit && (
                                                <button onClick={() => openModal(faculty)} className="text-blue-600 hover:text-blue-800">
                                                    <Pencil className="w-4 h-4" />
                                                </button>
                                            )}
                                            {canDelete && (
                                                <button onClick={() => handleDelete(faculty.id)} className="text-red-600 hover:text-red-800">
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center">
                            <h2 className="text-xl font-bold">{isEditing ? 'Edit Faculty' : 'Add New Faculty'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <Input name="teachingExp" value={formData.teachingExp} onChange={handleInputChange} placeholder="e.g. 5 Years" />
                            </div>
                            <div className="md:col-span-2">
